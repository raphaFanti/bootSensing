...
*/
//USE Serial FOR BLE CONNECTION WITH ADALOGGER
...
const int DOUT1 = A1; //strain gauge 1 4A
const int CLK = 5; //strain gauge 1 4A

const int DOUT2 = A2; //strain gauge 2 4B
//const int CLK2 = 10; //strain gauge 2 4B

const int DOUT3 = 6; //strain gauge 3 data LOAD CELL
//const int CLK1 = 12; //strain gauge 3 clock LOAD CELL

const int DOUT4 = 9; //strain gauge 4 data
//const int CLK4 = 5; //strain gauge 4 clock -> to define Pin as input

const int DOUT5 = 10; //strain gauge 5 data

const int DOUT6 = 11; //strain gauge 6 data

//const int DOUT7 = 12; //strain gauge 7 data

//const int DOUT8 = 13; //strain gauge 8 data
...
const int buttonPin_base = A0; //button pin for baseline
...
//Functions for hx711 Q2HX711
Q2HX711 strain1(DOUT1,CLK); //strain gauge 1 
Q2HX711 strain2(DOUT2,CLK); //strain gauge 2
Q2HX711 strain3(DOUT3,CLK); //strain gauge 3
Q2HX711 strain4(DOUT4,CLK); //strain gauge 4
Q2HX711 strain5(DOUT5,CLK); //strain gauge 5
Q2HX711 strain6(DOUT6,CLK); //strain gauge 6
//Q2HX711 strain7(DOUT7,CLK); //strain gauge 7
//Q2HX711 strain8(DOUT8,CLK); //strain gauge 8
...
//Construct filename (maxiumum digit # of filename = 12)
  ...
  File dataFile = SD.open(filename,FILE_WRITE); //open file 
   ...
   String header = "ID, strain1, strain2, temp [°C], runtime [ms]"; // file header
...
void loop() {
  ...
  float strain5out = strain5.read(); // reading HX711 from strain gauge 5
  float strain6out = strain6.read(); // reading HX711 from strain gauge 6
  //float strain7out = strain7.read(); // reading HX711 from strain gauge 7
  //float strain8out = strain8.read(); // reading HX711 from strain gauge 8
  //int chk = DHT.read(DHT11_PIN);
  //int temp = DHT.temperature;
  //int humidity = DHT.humidity;
  DateTime now = rtc.now(); //date & time
  ...
  String dataoutput = String(id) + ", " + String(strain1out) + ", "+ String(strain2out)+ ", " + String(strain3out) + ", " + String(strain4out) + ", " + String(strain5out) + ", " + String(strain6out) + ", " + String(temp) + ", " + String(t);
